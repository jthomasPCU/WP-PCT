	...
	class WP_Webhooks_Integrations_wordpress_Helpers_post_helpers {
		...
		public function manage_post_meta_data( $post_id, $post_meta_data ){
			...
			if( ! empty( $post_meta_data ) ){
				...
				if( is_array( $post_meta_data ) ){
					foreach( $post_meta_data as $function => $meta_data ){
						switch( $function ){
							...
							break;
							case 'wp_meta_fields':
								if( ! isset( $response['data']['wp_meta_fields'] ) ){
									$response['data']['wp_meta_fields'] = array();
								}

								foreach( $meta_data as $meta_key => $meta_data ){

									//clean the post meta value
									...
									if( is_array( $meta_data ) ){
										...
										foreach( $meta_data as $meta_value ){
											...
											$add_response = add_post_meta( $post_id, $meta_key, $meta_value );
				...
				$response['msg'] = __( 'No custom post meta given.', 'wp-webhooks' );
			}
