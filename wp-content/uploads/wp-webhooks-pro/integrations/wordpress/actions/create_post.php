...
if ( ! class_exists( 'WP_Webhooks_Integrations_wordpress_Actions_create_post' ) ) :
	...
	public function get_details(){
		...
		$parameter = array(
			...
			'post_date'			 => array( 'short_description' => __( '(string) The date of the post. Default is the current time. Format: 2018-12-31 11:11:11', 'wp-webhooks' ) ),
			...
			'post_status'		   => array(
				...
				'description' => __( "The post status defines further details about how your post will be treated. By default, WordPress offers the following post statuses: <strong>draft, pending, private, publish</strong>. Please note that other plugins can extend the post status values to offer a bigger variety, e.g. Woocommerce.", 'wp-webhooks' ),
			),
			'post_type'			 => array(
				'short_description' => __( '(string) The post type. Default \'post\'.', 'wp-webhooks' ),
				'description' => __( "The post type determines to which group of posts your currently created post belongs. Please use the slug of the post type to assign it properly.", 'wp-webhooks' ),
			),
			...
			'import_id'			 => array(
				'short_description' => __( '(integer) In case you want to give your post a specific post id, please define it here.', 'wp-webhooks' ),
				'description' => __( "This argument allows you to define a suggested post ID for your post. In case the ID is already taken, the post will be created using the default behavior by asigning automatically an ID. ", 'wp-webhooks' ),
			),
			'post_category'		 => array( 'short_description' => __( '(string) A comma separated list of category IDs. Defaults to value of the \'default_category\' option. Example: cat_1,cat_2,cat_3', 'wp-webhooks' ) ),
...
<pre>
{
	"meta_key": "Meta Value"
		...
		ob_start();
		?>
<?php echo __( "This argument is specifically designed to add/update or remove post meta to your created post.", 'wp-webhooks' ); ?>
					<br>
...
<ol>
	...
	<li><?php echo __( "The webhook response contains a validted array that shows each initialized meta entry, as well as the response from its original WordPress function. This way you can see if the meta value was adjusted accordingly.", 'wp-webhooks' ); ?></li>
		...
		ob_start();
		?>
<?php echo __( "The do_action argument is an advanced webhook for developers. It allows you to fire a custom WordPress hook after the create_post action was fired.", 'wp-webhooks' ); ?>
<br>
...
<ol>
	<li>
		...
		<?php echo __( "Contains the data that is used to create the post and some additional data as the meta input.", 'wp-webhooks' ); ?>
	</li>
	<li>
		<strong>$post_id</strong> (integer)<br>
		<?php echo __( "Contains the post id of the newly created post. Please note that it can also contain a wp_error object since it is the response of the wp_insert_user() function.", 'wp-webhooks' ); ?>
	</li>
	<li>
...
<pre>
{
	"meta_key": "Meta Value"
		...
		$returns = array(
			...
			$description = array(
				'tipps' => array(
					__( "In case you want to create a post for a custom post type, you can do that by using the <strong>post_type</strong> argument.", 'wp-webhooks' ),
					__( "By default, we create each post in a draft state. If you want to directly publish a post, use the <strong>post_status</strong> argument and set it to <strong>publish</strong>.", 'wp-webhooks' ),
					__( "In case you want to set a short description for your post, you can use the <strong>post_excerpt</strong> argument.", 'wp-webhooks' ),
			...
			return array(
				'action'			=> 'create_post',
				'name'			  => __( 'Create post', 'wp-webhooks' ),
				'sentence'			  => __( 'create a post', 'wp-webhooks' ),
				'parameter'		 => $parameter,
				'returns'		   => $returns,
				'returns_code'	  => $returns_code,
				'short_description' => __( 'Insert/Create a post. You have all functionalities available from wp_insert_post', 'wp-webhooks' ),
